...
export class PythonVisitor extends BaseVisitor {
  ...
  getImports() {
    return [
      "from __future__ import annotations", // To support forward class references (some classes are referenced before they are defined). See https://mypy.readthedocs.io/en/stable/runtime_troubles.html#forward-references
  ...
  private FieldOrInputValueDefinition(
  ...
  NonNullType(node: NonNullTypeNode) {
    ...
    // We make types Optional by default in `ListType` and `NamedType`, and remove the Optional if you see them wrapped in a `NonNull`
  ...
  InterfaceTypeDefinition(node: InterfaceTypeDefinitionNode) {
    ...
    const implementingNodes = astNode.definitions
      ...
      .filter((otherNode) =>
        otherNode.interfaces?.find(
          (interfaceNode) => interfaceNode.name.value === node.name.value
        )
      );
  ...
  getModelRebuild() {
    return this.modelsToRebuild
      .map((modelName) => `${modelName}.model_rebuild()`)
      .join("\n");
  }
}
