...
function createAstTreeNodeFromSelection({
  ...
  if (
    ...
    fieldSelection.fields.some(field => field.type === 'fragment')
  ) {
  ...
  const result: AstTreeNode[] = possibleTypes.map(possibleType => {
    const children: AstTreeNode[] = [];
    const filteredFields: FieldDefinitionNode[] = [];

    const selections = fieldSelection.fields.reduce<FieldSelection[]>((carry, selection) => {
      if (selection.type === 'object' || selection.type === 'scalar') {
        carry.push(selection);
      }

      if (
        selection.type === 'fragment' &&
        (selection.onType === possibleType.name || selection.onType === schemaNode.name)
      ) {
    ...
    assert(possibleType.astNode != null);
    for (const field of possibleType.astNode.fields ?? []) {
      const fieldSelection = selections
        .filter(
          fragmenSelection =>
            fragmenSelection.type === 'object' || fragmenSelection.type === 'scalar',
        )
        .find(selection => selection.name === field.name.value);
      ...
      const fieldSchemaNode = schema.getType(fieldType.name.value);

      assertIsNotUndefined(fieldSchemaNode);
      if (isObjectType(fieldSchemaNode) || isInterfaceType(fieldSchemaNode)) {
