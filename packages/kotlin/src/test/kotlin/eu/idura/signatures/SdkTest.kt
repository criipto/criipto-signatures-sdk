...
private val documentFixture = DocumentInput(
...
class SdkTest {
    private val sdk = IduraSignaturesSDK(
        clientId = System.getenv("CRIIPTO_SIGNATURES_CLIENT_ID")
            ?: error("CRIIPTO_SIGNATURES_CLIENT_ID not set"),
        clientSecret = System.getenv("CRIIPTO_SIGNATURES_CLIENT_SECRET")
            ?: error("CRIIPTO_SIGNATURES_CLIENT_SECRET not set"),
    )

    @Test
    fun testCreateSignatureOrderAddSignatory() = runTest {
        val signatureOrder = sdk.createSignatureOrder(
        ...
        val signatory = sdk.addSignatory(
    ...
    @Test
    fun testCreateSignatureOrderWithForm() = runTest {
        val signatureOrder = sdk.createSignatureOrder(
            CreateSignatureOrderInput(
                title = "Kotlin sample signature order",
                expiresInDays = 1,
    ...
    @Test
    fun testChangeMaxSignatories() = runTest {
        val signatureOrder = sdk.createSignatureOrder(
            CreateSignatureOrderInput(
                title = "Kotlin sample signature order",
                expiresInDays = 1,
        ...
        val changed = sdk.changeSignatureOrder(
    ...
    @Test
    fun testListSignatureOrders() = runTest {
        ...
        val signatureOrders = sdk.listSignatureOrders(
            first = 1000,
